...
# LICITACIONES ABIERTAS = fecha cierre futura Y estado NO adjudicada/cerrada/suspendida
hoy = pd.Timestamp.now()
estados_cerrados = ["Adjudicada","Cerrada","Suspendida","Desierta","Revocada"]
df_abiertas = df[
    (df["FechaCierre"] > hoy) & 
...
with tab1:
    ...
    if len(df_filtrado) == 0:
        ...
        st.dataframe(
            df_filtrado[["CodigoExterno","Nombre","Estado","FechaCierre"]].sort_values("FechaCierre"),
...
with tab2:
    ...
    with col1:
        ...
        df_todos["Estado"].value_counts().plot(kind="bar", ax=ax1, color=["#2ecc71","#e74c3c","#3498db","#f39c12","#9b59b6"])
        plt.tight_layout()
        st.pyplot(fig1)
    with col2:
